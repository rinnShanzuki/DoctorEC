...
const OnlineClients = () => {
    ...
    const fetchClients = async () => {
        try {
            const { data: response, fromCache } = await cachedGet('/clients');
    ...
    return (
        <div className="dashboard">
            <div className="dashboard-header">
                <h1>Online Registered Clients</h1>
                <p className="dashboard-subtitle">View and manage registered client accounts</p>
            </div>

            <div style={{ marginBottom: '24px', display: 'flex', gap: '12px', alignItems: 'center' }}>
                <input
                    type="text"
                    placeholder="Search by name, email, account no., or contact..."
                    value={searchTerm}
                        ...
                        <tbody>
                            {paginatedClients.length > 0 ? paginatedClients.map(client => (
                                <tr key={client.id}>
                                    ...
                                    <td>
                                        <button
                                            onClick={() => navigate(`/admin/dashboard/clients/${client.id}`)}
                                            style={{
                                ...
                                <tr>
                                    <td colSpan="5" style={{ textAlign: 'center', padding: '40px', color: '#999' }}>
                                        {searchTerm ? 'No clients match your search.' : 'No registered clients found.'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                {sortedClients.length > 0 && (
                    <div style={{
                        ...
                        <span style={{ color: '#666', fontSize: '14px' }}>
                            Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, sortedClients.length)} of {sortedClients.length} clients
                        </span>
